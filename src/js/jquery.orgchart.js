...
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    init: function (opts) {
      ...
      if ($.type(data) === 'object') {
        var $root = $chart.append($('<ul class="nodes"><li class="hierarchy"></li></ul>')).find('li');
        ...
        .done(function(data, textStatus, jqXHR) {
          that.buildHierarchy($chart.append($('<ul><li class="hierarchy"></li></ul>')).find('.hierarchy'), that.options.ajaxURL ? data : that.attachRel(data, '00'), 0, that.options);
        })
      ...
      if (relation === 'parent') {
        ...
        if ($target.length) {
          if (!(($target.closest('.nodes').length && $target.closest('.nodes').is('.hidden')) ||
            ...
            ($target.parent().is('li') && ($target.closest('ul').is('.hidden') || $target.closest('.vertical').is('.hidden')))
          )) {
            return { 'exist': true, 'visible': true };
          }
          return { 'exist': true, 'visible': false };
    ...
    hideChildrenEnd: function (event) {
      ...
      if (event.data.isVerticalDesc) {
        ...
        event.data.lowerLevel.find('.vertical').addClass('hidden');
      }
      if (this.isInAction($node)) {
    ...
    hideChildren: function ($node) {
      ...
      var isVerticalDesc = $lowerLevel.is('.vertical') ? true : false;
      if (!isVerticalDesc) {
    ...
    showChildren: function ($node) {
      ...
      var isVerticalDesc = $levels.is('.vertical') ? true : false;
    ...
    buildParentNode: function ($currentRoot, data) {
      ...
      var $newRootWrapper = $('<ul><li class="hierarchy"></li></ul>')
